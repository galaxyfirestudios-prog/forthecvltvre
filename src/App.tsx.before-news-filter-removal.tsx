...
export default function App() {
  ...
  {["LATEST", "NIGERIA", "AFRICA", "WORLD", "ENTERTAINMENT", "MUSIC", "CULTURE", "BUSINESS", "TECHNOLOGY", "SPORTS"].map((label) => (
    <button
      key={label}
      type="button"
      className={newsFilter === label ? "is-active" : ""}
      onClick={() => setNewsFilter(label)}
    >
      {label}
    </button>
  ))}
